...
// APIKEY=4c18a1e6fbb746c0bece299dc9a2fc85
...
function success(position) {
    ...
    var queryURL = "https://api.openweathermap.org/data/2.5/weather?lat=" + lat + "&lon=" + lon + "&APPID=4c18a1e6fbb746c0bece299dc9a2fc85";
    $.ajax({
        url: queryURL,
        method: "GET"
...
function showPrevious() {
    ...
    if (savedLocations) {
        $("#prevSearches").empty();
        var btns = $("<div>").attr("class", "list-group-flush");
        for (var i = 0; i < savedLocations.length; i++) {
            var locBtn = $("<a>").attr("href", "#").attr("id", "loc-btn").text(savedLocations[i]);
            if (savedLocations[i] == currentLoc){
                locBtn.attr("class", "list-group-item list-group-item-action btn-light active");
            }
            else {
...
function getCurrent(city) {
    var queryURL = "https://api.openweathermap.org/data/2.5/weather?q=" + city + "&APPID=4c18a1e6fbb746c0bece299dc9a2fc85&units=imperial";
    $.ajax({
        url: queryURL,
        method: "GET",
        error: function (){
        ...
        //create the card
        var currCard = $("<div>").attr("class", "card border-light mb-3");
        $("#currentforecast").append(currCard);

        // //add location to card header
        // var currCardHead = $("<div>").attr("class", "card-body").text("Current weather for " + response.name);
        ...
        //get UV Index
        var uvURL = "https://api.openweathermap.org/data/2.5/uvi?appid=4c18a1e6fbb746c0bece299dc9a2fc85&lat=" + response.coord.lat + "&lon=" + response.coord.lat;
        $.ajax({
            url: uvURL,
            method: "GET"
...
function getForecast(city) {
    //get 5 day forecast
    var queryURL = "https://api.openweathermap.org/data/2.5/forecast?id=" + city + "&APPID=4c18a1e6fbb746c0bece299dc9a2fc85&units=imperial";
    $.ajax({
        url: queryURL,
        method: "GET"
    }).then(function (response) {
        ...
        for (var i = 0; i < response.list.length; i++) {
            if (response.list[i].dt_txt.indexOf("15:00:00") !== -1) {
                ...
                // var newRow = $("<div>").attr("class", "row no-gutters");
